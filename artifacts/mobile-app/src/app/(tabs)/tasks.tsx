...
function TaskItem({ task, onPress }: { task: { id: string; type: string; priority: string; title: string; subtitle: string; action: string }; onPress: () => void }) {
  ...
  return (
    <Pressable onPress={onPress} style={{
      backgroundColor: s.bg, borderRadius: 14, padding: 14, borderWidth: 1, borderColor: s.border,
      ...
      {groups.map((g) => (
        <View key={g.priority}>
          <Pressable onPress={() => toggle(g.priority)} style={{ flexDirection: "row", alignItems: "center", gap: 8, paddingVertical: 8 }}>
            ...
            <Text style={{ fontSize: 12, fontWeight: "800", color: "#0F172A", flex: 1, textTransform: "uppercase", letterSpacing: 0.5 }}>
              {labels[g.priority] ?? g.priority}
            </Text>
