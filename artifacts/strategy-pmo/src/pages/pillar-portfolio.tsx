...
function PillarStatusBadge({ project }: { project: SpmaPillarPortfolioProject }) {
...
function PillarProjectStatusPieChart({ projects }: { projects: SpmaPillarPortfolioProject[] }) {
...
export default function PillarPortfolio({ params }: Props) {
  const pillarId = parseInt(params?.id ?? "0");
  ...
  if (isError || !data) {
    return (
      <div className="p-6">
        <Card className="text-center py-12 text-slate-500">Pillar not found.</Card>
      </div>
    );
  }

  const { pillar, initiatives } = data;
  const allProjects = initiatives.flatMap((i) => i.projects);
  ...
  const filteredInitiatives = initiatives.map((initiative) => ({
    ...initiative,
    projects: statusFilter === "all"
      ? initiative.projects
      : initiative.projects.filter((p) => classifyProject(p) === statusFilter),
  })).filter((i) => i.projects.length > 0 || statusFilter === "all");

  const filteredProjectCount = filteredInitiatives.reduce((s, i) => s + i.projects.length, 0);
    ...
    <div className="p-6 space-y-6">
      ...
      <div className="flex items-center gap-3">
        ...
        <div className="flex-1">
          ...
          <PageHeader
            title={pillar.name}
            description={`Pillar portfolio · ${allProjects.length} project${allProjects.length !== 1 ? "s" : ""} across ${initiatives.length} initiative${initiatives.length !== 1 ? "s" : ""}`}
          />
        </div>
      </div>

      {/* Pillar progress bar */}
      <div className="rounded-[14px] border border-border bg-card p-5 shadow-sm" style={{ borderLeft: `4px solid ${pillar.color}` }}>
        <div className="flex items-center justify-between mb-2">
          <span className="text-sm font-semibold text-muted-foreground">Pillar Progress</span>
          <span className="text-2xl font-display font-bold" style={{ color: pillar.color }}>
      ...
      <div className="grid grid-cols-2 md:grid-cols-4 gap-4">
        <Card className="flex flex-col gap-1">
          ...
          <p className="text-2xl font-bold text-slate-800">{allProjects.length}</p>
          <p className="text-xs text-slate-400">{initiatives.length} initiative{initiatives.length !== 1 ? "s" : ""}</p>
        </Card>
        <Card className="flex flex-col gap-1">
          <div className="flex items-center gap-2 text-slate-500 text-sm">
      ...
      {/* Pie chart */}
      {allProjects.length > 0 && <PillarProjectStatusPieChart projects={allProjects} />}

      {/* Initiatives + projects */}
      {allProjects.length === 0 ? (
        <Card className="text-center py-12">
          <FolderOpen className="w-10 h-10 text-slate-300 mx-auto mb-2" />
          <p className="text-slate-500 font-medium">No projects under this pillar</p>
        </Card>
      ) : (
        <>
          {/* Filter bar */}
          <div className="flex flex-wrap items-center gap-3">
            ...
            <span className="ml-auto text-xs text-muted-foreground">
              {filteredProjectCount} of {allProjects.length} projects
            </span>
          </div>
              ...
              {filteredInitiatives.map((initiative) => {
                ...
                return (
                  <div key={initiative.id} className="rounded-2xl border border-border overflow-hidden shadow-sm">
                        ...
                        <div className="flex-1 min-w-0">
                          ...
                          <p className="text-xs text-muted-foreground mt-0.5">
                            ...
                            {initiative.startDate && initiative.targetDate && (
                              <> · {initiative.startDate.slice(0, 10)} → {initiative.targetDate.slice(0, 10)}</>
                            )}
                          </p>
                        </div>
                        <div className="flex items-center gap-3">
                          <div className="text-right shrink-0">
                            ...
                            {initPlanned > 0 && (
                              <div className="text-[10px] text-muted-foreground">plan {initPlanned}%</div>
                            )}
                          </div>
                          <ChevronDown
                    ...
                    {isExpanded && (
                      <div className="divide-y divide-border">
                        {initiativeProjects.map((project) => {
                          ...
                          return (
                            <div key={project.id} className="px-6 py-4 bg-background hover:bg-secondary/10 transition-colors">
                              <div className="flex items-start justify-between gap-4">
                                <div className="min-w-0 flex-1">
                                  <div className="flex items-center gap-2 flex-wrap">
                                    ...
                                    >
                                      {project.projectCode && (
                                        <span className="font-mono text-slate-500 mr-1">{project.projectCode}:</span>
                                      )}
                                      {project.name}
                                    </button>
                                    <PillarStatusBadge project={project} />
                                  </div>
                                  {project.description && (
                    ...
                    {/* Collapsed hint */}
                    {!isExpanded && (
                      <div className="px-6 py-2 bg-background text-xs text-muted-foreground flex items-center gap-1">
                        <ChevronRight className="w-3 h-3" />
                        {initiativeProjects.length} project{initiativeProjects.length !== 1 ? "s" : ""} — click to expand
                      </div>
                    )}
                  </div>
                );
